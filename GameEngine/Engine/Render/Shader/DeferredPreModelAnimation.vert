...
readonly restrict layout(std430, binding = 3) buffer u_boneTransform
{
	mat4 boneTransforms[];
};


readonly restrict layout(std430, binding = 4) buffer u_vertexBoneData
{
...
void main()
{
    vertexBoneData[gl_VertexID];

    vec4 totalPosition = vec4(0);
    for(int i = 0; i < 4; i++)
	{
		if(vertexBoneData[gl_VertexID].boneIndices[i] == -1)
		   continue;

		vec4 localPosition = boneTransforms[vertexBoneData[gl_VertexID].boneIndices[i]] * vec4(vs_in_pos, 1);
        totalPosition += localPosition * vertexBoneData[gl_VertexID].boneWeights[i];
        //Normal?
	}
